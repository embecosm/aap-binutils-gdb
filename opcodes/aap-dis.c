...
void
aap_cgen_print_operand (CGEN_CPU_DESC cd,
			   int opindex,
			   void * xinfo,
			   CGEN_FIELDS *fields,
			   ...
			   int length)
{
  ...
  switch (opindex)
    ...
    case AAP_OPERAND_XDEST :
      print_keyword (cd, info, & aap_cgen_opval_gr_names, fields->f_long_dst_reg, 0|(1<<CGEN_OPERAND_VIRTUAL));
      break;
    case AAP_OPERAND_XSRC1 :
      print_keyword (cd, info, & aap_cgen_opval_gr_names, fields->f_long_src_reg_1, 0|(1<<CGEN_OPERAND_VIRTUAL));
      break;
    case AAP_OPERAND_XSRC2 :
      print_keyword (cd, info, & aap_cgen_opval_gr_names, fields->f_long_src_reg_2, 0|(1<<CGEN_OPERAND_VIRTUAL));
      break;
...
cgen_print_fn * const aap_cgen_print_handlers[] = 
{
  print_insn_normal,
};
...
void
aap_cgen_init_dis (CGEN_CPU_DESC cd)
{
  aap_cgen_init_opcode_table (cd);
  aap_cgen_init_ibld_table (cd);
  cd->print_handlers = & aap_cgen_print_handlers[0];
  cd->print_operand = aap_cgen_print_operand;
}
		   ...
		   int length)
{
  ...
  for (syn = CGEN_SYNTAX_STRING (syntax); *syn; ++syn)
      ...
      if (CGEN_SYNTAX_CHAR_P (*syn))
	{
      ...
      /* We have an operand.  */
      aap_cgen_print_operand (cd, CGEN_SYNTAX_FIELD (*syn), info,
...
#define CGEN_BFD_ARCH bfd_arch_aap
  ...
  if (! cd)
	...
	abort ();
      ...
      prev_mach = mach;
      prev_endian = endian;
      cd = aap_cgen_cpu_open (CGEN_CPU_OPEN_ISAS, prev_isa,
