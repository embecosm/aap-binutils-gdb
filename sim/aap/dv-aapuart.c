...
#define AAP_UART_RBR           0x0
#define AAP_UART_THR           0x0
#define AAP_UART_IER           0x4
#define AAP_UART_IIR           0x8
#define AAP_UART_LCR           0xc
#define AAP_UART_MCR           0x10
#define AAP_UART_LSR           0x14
#define AAP_UART_MSR           0x18
#define AAP_UART_DIV           0x1c

#define AAP_UART_IER_RX_INT    0x1
#define AAP_UART_IER_TX_INT    0x2

#define MICOUART_IIR_TXRDY      0x2
#define MICOUART_IIR_RXRDY      0x4

#define AAP_UART_LSR_RX_RDY    0x01
#define AAP_UART_LSR_TX_RDY    0x20

#define AAP_UART_LCR_WLS_MASK  0x3
#define AAP_UART_LCR_WLS_5     0x0
#define AAP_UART_LCR_WLS_6     0x1
#define AAP_UART_LCR_WLS_7     0x2
#define AAP_UART_LCR_WLS_8     0x3
...
static const struct hw_port_descriptor aapuart_ports[] = {
...
{
  struct aapuart *uart = hw_data (me);
  char c;

  /* Generate interrupt when transmission is complete.  */
  if (uart->ier & AAP_UART_IER_TX_INT)
    {
      /* Generate interrupt */
  ...
  uart->lsr |= AAP_UART_LSR_TX_RDY;

  /* Output the character in the THR.  */
  c = (char) uart->thr;

  /* WLS field in LCR register specifies the number of bits to output.  */
  switch (uart->lcr & AAP_UART_LCR_WLS_MASK)
    {
    case AAP_UART_LCR_WLS_5:
      c &= 0x1f;
      break;
    case AAP_UART_LCR_WLS_6:
      c &= 0x3f;
      break;
    case AAP_UART_LCR_WLS_7:
      c &= 0x7f;
      break;
    }
  printf ("%c", c);
...
static unsigned
aapuart_io_write_buffer (struct hw *me,
			  const void *source,
			  int space, unsigned_word base, unsigned nr_bytes)
{
  ...
  switch (uart_reg)
	  ...
	  && (uart->lsr & AAP_UART_LSR_TX_RDY))
	{
      ...
      else if ((value & AAP_UART_IER_TX_INT) == 0)
	{
...
static unsigned
aapuart_io_read_buffer (struct hw *me,
			 void *dest,
			 int space, unsigned_word base, unsigned nr_bytes)
{
  struct aapuart *uart = hw_data (me);
...
static void
attach_aapuart_regs (struct hw *me, struct aapuart *uart)
{
...
static void
aapuart_finish (struct hw *me)
{
  struct aapuart *uart;
  int i;

  uart = HW_ZALLOC (me, struct aapuart);
  set_hw_data (me, uart);
  set_hw_io_read_buffer (me, aapuart_io_read_buffer);
  set_hw_io_write_buffer (me, aapuart_io_write_buffer);
  set_hw_ports (me, aapuart_ports);

  /* Attach ourself to our parent bus.  */
  attach_aapuart_regs (me, uart);
...
const struct hw_descriptor dv_aapuart_descriptor[] = {
  {"aapuart", aapuart_finish,},
  {NULL},
};
