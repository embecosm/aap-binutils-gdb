/* aap16 simulator support code  */

#define WANT_CPU aap16bf
#define WANT_CPU_AAP16BF
...
int
aap16bf_fetch_register (SIM_CPU *current_cpu, int rn, unsigned char *buf, int len)
{
  return aapbf_fetch_register (current_cpu, rn, buf, len);
}

/* The contents of BUF are in target byte order.  */

int
aap16bf_store_register (SIM_CPU *current_cpu, int rn, unsigned char *buf, int len)
{
  return aapbf_store_register (current_cpu, rn, buf, len);
}

/* Cover fns to get/set the control registers.
   FIXME: Duplicated from aap.c.  The issue is structure offsets.  */

USI
aap16bf_h_cr_get_handler (SIM_CPU *current_cpu, UINT cr)
{
  switch (cr)
    {
    case H_CR_PSW : /* PSW.  */
...
void
aap16bf_h_cr_set_handler (SIM_CPU *current_cpu, UINT cr, USI newval)
{
  switch (cr)
    {
    case H_CR_PSW : /* psw */
...
UQI
aap16bf_h_psw_get_handler (SIM_CPU *current_cpu)
{
  return (CPU (h_psw) & 0xfe) | (CPU (h_cond) & 1);
}

void
aap16bf_h_psw_set_handler (SIM_CPU *current_cpu, UQI newval)
{
  CPU (h_psw) = newval;
  CPU (h_cond) = newval & 1;
}
...
DI
aap16bf_h_accum_get_handler (SIM_CPU *current_cpu)
{
...
void
aap16bf_h_accum_set_handler (SIM_CPU *current_cpu, DI newval)
{
  CPU (h_accum) = newval;
}
...
DI
aap16bf_h_accums_get_handler (SIM_CPU *current_cpu, UINT regno)
{
...
void
aap16bf_h_accums_set_handler (SIM_CPU *current_cpu, UINT regno, DI newval)
{
  /* FIXME: Yes, this is just a quick hack.  */
  if (regno == 0)
    CPU (h_accum) = newval;
...
void
aap16bf_model_insn_before (SIM_CPU *cpu, int first_p)
{
  aapbf_model_insn_before (cpu, first_p);
}
...
void
aap16bf_model_insn_after (SIM_CPU *cpu, int last_p, int cycles)
{
  aapbf_model_insn_after (cpu, last_p, cycles);
}
...
int
aap16bf_model_aap16_u_exec (SIM_CPU *cpu, const IDESC *idesc,
...
int
aap16bf_model_aap16_u_cmp (SIM_CPU *cpu, const IDESC *idesc,
			   int unit_num, int referenced,
			   INT src1, INT src2)
{
...
int
aap16bf_model_aap16_u_mac (SIM_CPU *cpu, const IDESC *idesc,
			   int unit_num, int referenced,
			   INT src1, INT src2)
{
  check_load_stall (cpu, src1);
...
int
aap16bf_model_aap16_u_cti (SIM_CPU *cpu, const IDESC *idesc,
			  int unit_num, int referenced,
			  INT sr)
{
...
int
aap16bf_model_aap16_u_load (SIM_CPU *cpu, const IDESC *idesc,
			   int unit_num, int referenced,
			   INT sr, INT dr)
{
...
int
aap16bf_model_aap16_u_store (SIM_CPU *cpu, const IDESC *idesc,
			    int unit_num, int referenced,
			    INT src1, INT src2)
{
  return idesc->timing->units[unit_num].done;
