/* Collection of junk waiting time to sort out -- aap  */
...
extern int aap_decode_gdb_ctrl_regnum (int);
...
#ifndef GET_H_CR
extern USI  aapbf_h_cr_get_handler (SIM_CPU *, UINT);
extern void aapbf_h_cr_set_handler (SIM_CPU *, UINT, USI);
...
#ifndef  GET_H_PSW
extern UQI  aapbf_h_psw_get_handler (SIM_CPU *);
extern void aapbf_h_psw_set_handler (SIM_CPU *, UQI);
...
#ifndef  GET_H_ACCUM
extern DI   aapbf_h_accum_get_handler (SIM_CPU *);
extern void aapbf_h_accum_set_handler (SIM_CPU *, DI);
...
void aap_init_insn_cycles (SIM_CPU *, int);
/* Update the totals for the insn.  */
void aap_record_insn_cycles (SIM_CPU *, int);
...
do { \
  if (PROFILE_INSN_P (cpu)) \
    ++ CPU_AAP_MISC_PROFILE (cpu)->short_count; \
} while (0)
...
do { \
  if (PROFILE_INSN_P (cpu)) \
    ++ CPU_AAP_MISC_PROFILE (cpu)->long_count; \
} while (0)

...
#ifdef AAP_EVA /* orig eva board, no longer supported */
...
#define AAP_DEVICE_ADDR	0xff000000
#define AAP_DEVICE_LEN		0x00ffffff
...
USI aap_trap (SIM_CPU *, PCADDR, int);
