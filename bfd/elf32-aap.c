...
#define AAP_GET_MEMORY_SPACE_ID(ADDR) (((ADDR) >> (32 - 7)) & 0xff)
#define AAP_GET_ADDRESS_LOCATION(ADDR) ((ADDR) & 0x01ffffff)
#define AAP_BUILD_ADDRESS(ID,LOC) (((ID & 0xff) << (32 - 7)) | (LOC & 0x01ffffff))
...
static reloc_howto_type elf_aap_howto_table[] =
{
  ...
  HOWTO (R_AAP_FORCEPCREL16,   /* Type.  */
         ...
         complain_overflow_signed, /* Complain_on_overflow.  */
         bfd_elf_generic_reloc, /* Special_function.  */
         "R_AAP_FORCEPCREL16", /* Name.  */
         TRUE,                  /* Partial_inplace.  */
         0xffff,                /* Src_mask.  */
         0xffff0000,            /* Dst_mask.  */
         FALSE),                /* PCrel_offset.  */
  HOWTO (R_AAP_FORCEPCREL8,    /* Type.  */
         0,                     /* Rightshift.  */
         1,                     /* Size (0 = byte, 1 = short, 2 = long).  */
         8,                     /* Bitsize.  */
         TRUE,                  /* PC_relative.  */
         ...
         complain_overflow_signed, /* Complain_on_overflow.  */
         bfd_elf_generic_reloc, /* Special_function.  */
         "R_AAP_FORCEPCREL8",  /* Name.  */
         TRUE,                  /* Partial_inplace.  */
         0xff,                  /* Src_mask.  */
         0x00ff,                /* Dst_mask.  */
         FALSE),                /* PCrel_offset.  */
  HOWTO (R_AAP_CONST4,         /* Type.  */
         0,                     /* Rightshift.  */
         1,                     /* Size (0 = byte, 1 = short, 2 = long).  */
         4,                     /* Bitsize.  */
         FALSE,                 /* PC_relative.  */
...
struct elf_aap_section_data
{
  struct bfd_elf_section_data elf;
  struct aap_relax_info relax_info;
};

static struct aap_relax_info *
get_aap_relax_info (asection *sec)
{
  struct elf_aap_section_data *section_data;

  /* No info available if no section or if it is an output section.  */
  if (!sec || sec == sec->output_section)
    return NULL;

  section_data = (struct elf_aap_section_data *) elf_section_data (sec);
  return &section_data->relax_info;
}

static void
init_aap_relax_info (asection *sec)
{
  struct aap_relax_info *relax_info = get_aap_relax_info (sec);
...
static bfd_boolean
elf_aap_new_section_hook (bfd *abfd, asection *sec)
{
  if (!sec->used_by_bfd)
    {
      struct elf_aap_section_data *sdata;
...
{
  /* The AAP compiler does not place the machine type into the elf
  ...
  return bfd_default_set_arch_mach (abfd, bfd_arch_aap, bfd_mach_aap);
}

/* Helper function for AAP_FINAL_LINK_RELOCATE.  Called to adjust the
   value being patched into a R_AAP_CONST4 relocation.  The const4
   ...
   parameters being passed to AAP_FINAL_LINK_RELOCATE.  */

static bfd_vma
aap_final_link_relocate_const4 (bfd *input_bfd,
...
/* Perform a single relocation.
   ...
   used in these case _BFD_FINAL_LINK_RELOCATE, however, there is one AAP
                          ...
                          struct elf_link_hash_entry * h)
{
  ...
  /* For aap we use address space identifier bits merged in to the VMA in
  ...
  relocation_memory_id = AAP_GET_MEMORY_SPACE_ID (relocation);

  if (howto->pc_relative
      && relocation_memory_id != 0
      && relocation_memory_id != AAP_GET_MEMORY_SPACE_ID (address))
    _bfd_error_handler
      (_("warning: %B relocation at %s + 0x%"BFD_VMA_FMT
         "x is pc-relative across address spaces, %#08lx to %#08lx"),
       input_bfd, input_section->name, offset, address, relocation);

  relocation = AAP_GET_ADDRESS_LOCATION (relocation);

  /* The special R_AAP_CONST4 relocation uses a mapping table to encode it's
     value.  Only some values can be handled by a R_AAP_CONST4
     relocation.  */
  if (howto->type == R_AAP_CONST4)
    relocation = aap_final_link_relocate_const4 (input_bfd, input_section,
                                                  offset, relocation);
  ...
  address_location = AAP_GET_ADDRESS_LOCATION (address);
  if (howto->pc_relative)
  ...
  if (howto->type == R_AAP_CALL16
      || howto->type == R_AAP_CALL14)
    {
  ...
  if ((input_section->flags & SEC_DEBUGGING) != 0
    ...
    relocation = AAP_BUILD_ADDRESS (relocation_memory_id, relocation);
...
relax_log (const char *fmt, ...)
{
  ...
  if (relaxation_logging == UNINITIALISED)
    {
      const char *var = getenv ("AAP_RELAXATION_LOGGING");
      if (var && (*var == 'y' || *var == 'Y'))
...
static bfd_boolean
aap_elf_relax_delete_bytes (bfd *abfd,
                             asection *sec,
                             bfd_vma addr,
                             int count)
{
  ...
  relax_info = get_aap_relax_info (sec);
...
/* Insert COUNT bytes into the contents of SEC (from ABFD) at section
   ...
   by aap_elf_relax_delete_bytes, as we don't allocate new memory for
...
static bfd_boolean
aap_elf_relax_insert_bytes (bfd *abfd,
                             asection *sec,
                             bfd_vma addr,
                             int count)
{
...
static bfd_boolean
aap_relax_section_filter (bfd *abfd, asection *sec)
{
...
static bfd_boolean
aap_relax_relocation_filter (unsigned int type)
{
  return (type == R_AAP_PCREL16
          || type == R_AAP_HIGH16
          || type == R_AAP_CALL16);
}
...
static bfd_boolean
aap_relax_handle_relocation (bfd *abfd, asection *sec,
    ...
    case R_AAP_PCREL16:
      {
        ...
        if (AAP_GET_MEMORY_SPACE_ID (reloc_addr)
            != AAP_GET_MEMORY_SPACE_ID (dest_addr))
          return TRUE;

        /* Let's not worry about address space ID any more.  */
        reloc_addr = AAP_GET_ADDRESS_LOCATION (reloc_addr);
        dest_addr = AAP_GET_ADDRESS_LOCATION (dest_addr);
        ...
        if (!aap_elf_relax_delete_bytes (abfd, sec,
                                          irel->r_offset + 2, 2))
          return FALSE;
...
static bfd_boolean
aap_check_section_filter (bfd *abfd, asection *sec)
{
  struct aap_relax_info *relax_info;
...
static bfd_boolean
aap_check_relocation_filter (unsigned int type)
{
  return (type == R_AAP_PCREL8 || type == R_AAP_CALL14);
}
...
static bfd_boolean
aap_check_handle_relocation (bfd *abfd, asection *sec,
                              struct bfd_link_info *link_info,
                              Elf_Internal_Rela *irel,
                              bfd_vma symval,
                              Elf_Internal_Rela *internal_relocs,
    ...
    case R_AAP_PCREL8:
      {
        ...
        if (!aap_elf_relax_insert_bytes (abfd, sec,
                                          irel->r_offset + 2, 2))
          return FALSE;
...
static struct aap_relaxation_hooks aap_relax_hooks =
  {
    aap_relax_section_filter,
    aap_relax_relocation_filter,
    aap_relax_handle_relocation
  };

/* The functions to check that previous relaxation are still valid.  */

static struct aap_relaxation_hooks aap_check_hooks =
  {
    aap_check_section_filter,
    aap_check_relocation_filter,
    aap_check_handle_relocation
  };
...
static bfd_boolean
aap_elf_relax_section_worker (bfd *abfd,
                               asection *sec,
                               struct bfd_link_info *link_info,
                               bfd_boolean *again,
                               struct aap_relaxation_hooks *hooks)
{
  Elf_Internal_Shdr *symtab_hdr;
  Elf_Internal_Rela *internal_relocs;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      ...
      if (!hooks->reloc_filter (ELF32_R_TYPE (irel->r_info)))
        continue;

      BFD_ASSERT (ELF32_R_TYPE (irel->r_info) < (unsigned int) R_AAP_max);
      howto = &elf_aap_howto_table[ELF32_R_TYPE (irel->r_info)];
...
static bfd_boolean
aap_elf_relax_section (bfd *abfd,
                        asection *sec,
                        struct bfd_link_info *link_info,
                        bfd_boolean *again)
{
  ...
  *again = FALSE;
  ...
  /* Due to the very strange way in which linker relaxation is triggered on
     ELF files from `gldelf32aap_map_segments` the whole linker relaxation
  ...
  switch (link_info->relax_pass)
    ...
    case 0:
      answer =
        aap_elf_relax_section_worker (abfd, sec, link_info, again,
                                       &aap_relax_hooks);
      break;
...
#define bfd_elf32_bfd_relax_section         aap_elf_relax_section
#define bfd_elf32_new_section_hook	    elf_aap_new_section_hook
